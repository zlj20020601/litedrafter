#!/usr/bin/env python3
"""
CodeContests benchmark 数据集生成 — Qwen3.5-4B 版本
规格:
  ...
  - Tokenizer: Qwen3.5-4B (vocab 248044)
...
与 Qwen3-8B 版本的区别:
  1. MODEL_PATH → Qwen3.5-4B
  2. enable_thinking=False 在 Qwen3.5 中多了 <think>\n\n</think>\n\n 后缀 (+2 tokens)
     → content 需要少约 2 tokens 才能命中精确 1024
  3. 分桶区间不变：A=[1024,1152], B1=[1153,1280]
"""
...
LOG_FILE   = os.path.join(LOG_DIR, "codecontests_qwen35_gen.log")
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "codecontests_qwen35_4b_1024_256.jsonl")
META_FILE   = os.path.join(OUTPUT_DIR, "codecontests_qwen35_4b_1024_256.meta.json")
...
def fit_to_target(tok, content, target=TARGET):
    ...
    策略：token 级粗调 → keep 邻域扫描 → 字符级微调 → 字符二分兜底。
...
def main():
    ...
    log("=" * 70)
    log("CodeContests benchmark 数据生成 — Qwen3.5-4B 版本")
    log(f"Tokenizer: {MODEL_PATH}")
    log(f"Selection: {A_SIZE}×[1024,1152] + {B1_SIZE}×[1153,1280], seed={SEED}")
    log(f"Chat template: enable_thinking=False, add_generation_prompt=True")
    log("=" * 70)

    from transformers import AutoTokenizer
    tok = AutoTokenizer.from_pretrained(MODEL_PATH, trust_remote_code=True)
    log(f"tokenizer: vocab_size={tok.vocab_size}, len={len(tok)}")
    ...
    log(f"parquet shards: {len(files)}")
    if not files:
        log("FATAL: CodeContests cache not found. Run dataset_survey.py first.")
        sys.exit(1)
    ...
    log(f"raw length: min={min(raw_lens)}, median={sorted(raw_lens)[len(raw_lens)//2]}, max={max(raw_lens)}")
    ...
    if len(buckets["A"]) < A_SIZE:
        log(f"WARNING: A 桶不足 {A_SIZE}，实际 {len(buckets['A'])}，全部使用")
    ...
    results = []
    stats = {"raw_lt_target": 0, "truncated": 0, "iter_ok": 0, "fallback_used": 0}
    ...
    log(f"成功生成 {len(results)} 条 (truncated={stats['truncated']}, "
        f"len==1024={stats['iter_ok']}, fallback={stats['fallback_used']})")
    ...
    meta = {
        ...
        "chat_template": "Qwen3.5, enable_thinking=False, add_generation_prompt=True",
        "chat_template_suffix": "<|im_start|>assistant\\n<think>\\n\\n</think>\\n\\n",
        "selection": {"A_1024_1152": len(selected_a), "B1_1153_1280": len(selected_b1), "seed": SEED},
        "target_length": TARGET,
        "num_prompts": len(results),
        "source_distribution": dict(Counter(r["source"] for r in results)),
        "truncated_count": stats["truncated"],
        "raw_length_stats": {"min": min(raw_lens), "median": sorted(raw_lens)[len(raw_lens)//2], "max": max(raw_lens)},
        ...
        "difference_from_qwen3_8b_version": "Qwen3.5 enable_thinking=False adds </think> suffix (+2 tokens); "
                                            "tokenizer is different (vocab 248044 vs 151936)",
    }
